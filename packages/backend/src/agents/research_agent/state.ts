...
export const SearchAgentStateAnnotation = Annotation.Root({
  ...
  search_queries: Annotation<string[]>({ // New: Queries generated by planner
    default: () => [],
    reducer: (s: string[], a: string[]) => s.concat(a),
  }),
  search_results: Annotation<Array<{ query: string; result: string }>>({ // New: Results from searches
    default: () => [],
    reducer: (s: Array<{ query: string; result: string }>, a: Array<{ query: string; result: string }>) => s.concat(a),
  }),
