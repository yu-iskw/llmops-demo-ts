...
const SPECIALIST_QUALITY_PROMPT = `
...
/** LLM judge: specialist answer quality vs reference rubric (shared across trip / finance / general). */
export const specialistQualityEvaluatorGenAI = async (
  run: Run,
  example?: Example,
) => {
  const evaluator = createGenAIAsJudge({
    prompt: SPECIALIST_QUALITY_PROMPT,
