import { createAndAddExamples } from "../../../../../eval/langfuse/datasets/tripSpecialistDataset";
...
export async function runLlmJudgeEvaluation() {
  ...
  try {
    ...
    const result = await dataset.runExperiment({
      name: "routed-agent-trip-specialist-evaluation",
      description:
        "Trip specialist offline evaluation (YAML synced to Langfuse).",
      ...
      maxConcurrency: 2,
      metadata: { suite: "trip_specialist", provider: "langfuse" },
    });

    console.log(await result.format());
    console.log(
      "Trip specialist Langfuse evaluation completed. Check Langfuse UI for results.",
    );
  } finally {
