import { createAndAddExamples } from "../../../../../eval/langfuse/datasets/financeSpecialistDataset";
...
export async function runLlmJudgeEvaluation() {
  ...
  try {
    ...
    const result = await dataset.runExperiment({
      name: "routed-agent-finance-specialist-evaluation",
      description:
        "Finance specialist offline evaluation (YAML synced to Langfuse).",
      ...
      maxConcurrency: 2,
      metadata: { suite: "finance_specialist", provider: "langfuse" },
    });

    console.log(await result.format());
    console.log(
      "Finance specialist Langfuse evaluation completed. Check Langfuse UI for results.",
    );
  } finally {
