...
7. **再ビルドと再起動** (モジュール 1 と同様の手順、`pnpm build:agents` と `pnpm start:backend` を使用)
...
- **TypeScript のコンパイルを確認**: `pnpm build:agents` を実行して型エラーを見つけます
...
1. **再ビルドを忘れる**: 変更後は必ず `pnpm build:agents` を実行してください
